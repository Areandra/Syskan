...
import { View, Text, TextInput, ScrollView, StyleSheet, TouchableOpacity, Alert, FlatList, Modal } from 'react-native';
...
const NewInvoiceScreen = ({ route, navigation }) => {
  ...
  const { visible, setVisible, visibleCar, setVisibleCar, visibleHeader, setVisibleHeader, visibleHarga, } = useGlobal()
  ...
  try {
    // 1. Simpan ke koleksi global "invoices"
    ...
    // 2. Simpan ke setiap user (role: Langganan)
    for (const item of items) {
      const userId = item.langganan; // Pastikan item.user adalah ID dokumen user
      await setDoc(doc(db, 'users', userId, 'invoices', `${draftKey} ${userId}`), {
        header,
        item
      });
    }

    // 3. Hapus dari AsyncStorage (local draft)
...
const animatedStyle = useAnimatedStyle(() => {
    return {
      opacity: withTiming(visible || visibleCar || visibleHeader || visibleHarga ? 0 : 1),  // fade out kalau isAtTop true
      transform: [
        {
          translateY: withTiming(visible || visibleCar || visibleHeader || visibleHarga ? 20 : 0),  // geser ke bawah sedikit
        },
      ],
      zIndex: 1,
    };
  }, [visible, visibleCar, visibleHeader, visibleHarga]);
...
return (
  ...
  animationType="slide"
  transparent={true}
  onRequestClose={() => setVisibleHarga(false)}
>
  <View style={styles.modalOverlay}>
    <View style={styles.modalContent}>
      <Text style={styles.modalTitle}>Atur Harga Ikan</Text>
      <ScrollView>
        {Object.entries(hargaMap).map(([key, value]) => (
          <View key={key} style={styles.hargaRow}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                ...
                setHargaMap(newHargaMap);
                setListHarga(newHargaMap); // update global state
              }}
            />
          </View>
        ))}
      </ScrollView>
      <TouchableOpacity onPress={() => setVisibleHarga(false)} style={styles.closeButton}>
    ...
    <Header title={header ? `${header?.pemasok} ${header?.sopir}` : ''} navigation={navigation} back={true} kembali={true} pemasok={true} langganan={true} />
    <View style={[styles.container, {overflow: 'visible'}]} >
      <View style={{flex:1, overflow: 'visible',}}>
      {(!visibleHeader && !visible && !visibleCar && !visibleHarga) && (
